...
| Command | Subcommands |
| --- | --- |
| `mindbridge` | `config check`, `consolidate`, `jobs`, `lifecycle`, `mcp`, `jina serve`, `sentence-transformers serve`, `edge sync` |
...
control the one model process. A non-default model or revision requires a new space. Repeat
`--media-origin` for every exact HTTP(S) origin that may serve presigned media; remote URLs from
any other origin and all redirects are rejected. `/health` is public. `/v1/models` and
`/v1/embeddings` require the bearer token. `mindbridge jina serve` remains an alias.
