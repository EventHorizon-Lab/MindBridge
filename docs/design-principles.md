...
[architecture](architecture.md#shared-execution-plane).
...
`TranscriptionBackend`, and `SpeechBackend` protocols. Constructor injection is the plugin mechanism
implemented today; there is no runtime plugin registry.
...
| Extensions | Four narrow model protocols; no registry | Optional domain capabilities after a real implementation establishes the contract |
