...
| Stream finality | `UPDATE` is speculative, `FINAL` persists once, and `CANCEL` or unfinished EOF writes nothing. Audio and vision adapters normalize provider events into the same boundary, and a configured vision describer's text is indexed for every embedder rather than only a text-only one. | [Async capture streams](api/python-sdk.md#async-capture-streams), [omni streaming](omni-streaming-and-interaction-memory.md) |
...
| Public surfaces | Supported Python values are imported from `mindbridge`; REST is under `/v1`; MCP exposes fourteen tools, the memory path plus the embodied and identity operations. Typed context added no tool, and optional context introduces no account, user, request, or benchmark scope. | [Python SDK](api/python-sdk.md), [REST](api/rest.md), [MCP](api/mcp.md) |
