...
the engine resolves from the device (CUDA with vLLM installed uses it, every other platform uses
FunASR `AutoModel`) and the recipe defaults to `fun-asr-nano`. Set `asr_recipe` when the
