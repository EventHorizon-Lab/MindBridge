...
# Month-first ("July 1, 2025", "July, 1 2025") and day-first ("1 July 2025", "1st of July") spellings
# of one stated clock. A comma that lands between the month and the day used to defeat the anchor
# entirely, so "Today is July, 1 2025, ... in the past two years" was read against the real
# wall clock instead of the date the question itself declared. Every month name ends at a word
# boundary and every day at a non-digit, so "Today is 2 decades" names no December and
# "Today is July 2025" reads no 20th out of the year.
...
# A rolling span ending at the reference clock: "past two years", "last 3 months", "recent 2
# weeks", "过去两年", "最近三个月". Only days used to be recognized, so a question about "the past
# two years" carried no window at all while "the past 730 days" did. The count is required:
# "recent years" and "the previous month" are adjectives, not spans, and "my recent day trips"
# must not become a one-day window. The one bare form kept is "the past <unit>" -- "in the past
# year" is one rolling year, the same reading the day grammar already gave "past week".
...
# A day a record's own text states, with or without its year: "june 23, 2022", "june 23-24,
# 2022", "23 june 2022", "1-2 june 2022", and the year-less "june 23" that borrows the year of
# the date it is joined to, as in "june 23 - july 2, 2022". Applied to casefolded text only, so
# no IGNORECASE. Read by `_stated_named_days`; the query side keeps its own clock grammar above.
...
# What may stand between a year-less day and the dated day whose year it takes.
_DATE_JOINER = re.compile(r"^\s*(?:[-\u2013]|to|and|through|until|till)\s*$")
_ANY_DIGIT = re.compile(r"\d")
# A record whose stated dates span more than this is stating dates about several things -- a
# booking plus its terms revision, a newsletter -- not one event, so its event time bounds it.
...
class _RecallReads:
    ...
    def time_span(self, rows: Sequence[SearchHit]) -> tuple[datetime, datetime] | None:
        """Bound a later read by these rows; see `_rows_time_span`."""
        return _rows_time_span(rows, self._context.reference)
...
def _parse_temporal_range(  # noqa: C901 - ordered phrases are clearer as one parser
    ...
    if not text:
        return None
    dates = _stated_iso_days(text)
    if dates:
        return _date_range(min(dates), max(dates), reference_at)

    normalized = text.casefold()
    months = _stated_months(normalized)
    if months:
    ...
    rolling = _rolling_span(normalized, reference_at)
...
def _stated_date_span(text: str, reference_at: datetime) -> tuple[datetime, datetime] | None:
    """The half-open span of the absolute dates one record's text states, or None.

    ISO days, `Month YYYY`, `Month D, YYYY`, `D Month YYYY`, a `D-D` range in either spelling,
    and a year-less day joined to a dated one (`June 23 - July 2, 2022`); a bare calendar year
    and every relative phrase are left out on purpose -- see `_rows_time_span`. Years run 1900
    through 2199, the range the rest of the temporal grammar accepts, so a stored sentinel such
    as `9999-12-31` is not a date the span can overflow on. Bounded to what the temporal parser
    already recognises, so a record cannot state a date the query side could not have asked for.
    """
    # Every recognised spelling carries a four-digit year, so a digit-free text -- most captions
    # -- is answered before any of the date grammars runs over it.
    if not text or _ANY_DIGIT.search(text) is None:
        return None
    normalized = text.casefold()
    days = [*_stated_iso_days(text), *_stated_named_days(normalized)]
    spans = [_date_range(day, day, reference_at) for day in days]
    dated_months = {(day.year, day.month) for day in days}
    for year, month in _stated_months(normalized):
        # "1 October 2024" also reads as "October 2024"; the stated day already bounds that month.
...
def _rows_time_span(
    ...
    """The half-open span a set of records covers, for a read bound to them.

    A record's stated dates win over its event time. A booking email is sent weeks before the
    stay it confirms, so a window on the emails' own `occurred_at` would hold the days the mail
    arrived and none of the trip; the dates in its text are the trip. A record that states no
    date contributes its event time, and one whose stated dates spread over more than
    `_MAX_STATED_SPAN` is stating dates about several things -- a booking beside its terms
    revision -- so it too falls back to its event time rather than widening the read to years.
    `None` when no row carries a time. A span that collapses to one instant is widened to the
    end of that day.
    """
    starts: list[datetime] = []
    ends: list[datetime] = []
    for hit in rows:
        stated = _stated_date_span(hit.content, reference_at)
        if stated is not None and stated[1] - stated[0] <= _MAX_STATED_SPAN:
            ...
            ends.append(hit.occurred_end or hit.occurred_at)
    if not starts:
        return None
...
def _stated_iso_days(text: str) -> list[date]:
    days = []
...
def _stated_named_days(normalized: str) -> list[date]:
    """Every fully stated day, both ends of a range and a borrowed year included.

    A day with no year of its own takes the year of the dated day it is joined to by a dash,
    "to", "and", or "through" -- the first half of "June 23 - July 2, 2022" -- and is dropped
    otherwise, because "may 5" in prose is not a date.
    """
...
def _stated_months(normalized: str) -> list[tuple[int, int]]:
    months = [
...
def _rolling_span(
    ...
    """Resolve "past N <unit>" to the span ending at the reference clock, or None.

    Every rolling phrase in the text is tried in order and the first that resolves wins, so
    "in the past few days, especially the last 3 days" reads the three days rather than giving
    up on "few". "last N <unit>" needs its count: bare "last year" and "last month" are the
    calendar phrases the caller resolves after this. A rolling month or year is a calendar shift,
    not a day count, so "past two years" asked on 30 August ends two 30 Augusts back; the day is
...
def _shift_month(value: datetime, months: int) -> datetime:
    ...
    Every calendar-phrase caller passes the first of a month, for which the clamp is a no-op; a
    rolling span shifts the reference clock itself, and 31 August minus six months is 28 (or 29)
