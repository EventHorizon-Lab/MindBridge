...
class RerankCandidate:
    """One caller-owned candidate with a stable opaque ID."""
    ...
    def __post_init__(self) -> None:
        if not self.candidate_id.strip():
            raise DomainInvariantError("rerank candidate ID must not be blank")


@dataclass(frozen=True, slots=True)
class RerankRequest:
    """A query and the complete candidate set to reorder."""
    ...
    def __post_init__(self) -> None:
        ...
        if not candidate_ids:
            raise DomainInvariantError("rerank request must contain candidates")
        if len(set(candidate_ids)) != len(candidate_ids):
            raise DomainInvariantError("rerank candidate IDs must be unique")


@dataclass(frozen=True, slots=True)
class RerankResult:
    """Every input candidate ID, in the model-selected order."""

    candidate_ids: tuple[str, ...]

    def __post_init__(self) -> None:
        if not self.candidate_ids or len(set(self.candidate_ids)) != len(self.candidate_ids):
            raise DomainInvariantError("rerank result IDs must be non-empty and unique")


@runtime_checkable
class Reranker(Protocol):
    """A replaceable provider or local reranking model."""
