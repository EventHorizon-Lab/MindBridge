...
class _ExemplarBank:
    ...
    def __init__(self, rows: Sequence[sqlite3.Row], *, dimension: int, modality: str) -> None:
        ...
        self._ids: list[str] = []
        self._created = [row_text(row, "created_at") for row in rows]
        owners = []
        for row in rows:
            identity_id = row_text(row, "identity_id")
        ...
        self._raw = np.frombuffer(
            b"".join(row_blob(row, "vector") for row in rows), dtype="<f4"
    ...
    def stored(self, identity_id: str) -> list[tuple[tuple[float, ...], str]]:
        ...
        return [
            (
                normalized_vector(tuple(float(value) for value in self._raw[row]), self._name),
                self._created[row],
            )
