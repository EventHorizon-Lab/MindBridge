...
def date_token_match(gt_token: str, pred_token: str) -> bool:
    if "-" in gt_token:
        ...
        return date_component_match(gt_start, pred_start) and date_component_match(gt_end, pred_end)
...
def _deterministic_accuracy_core(
    ...
    if gt_codes:
        ...
        pred_stripped = re.sub(r"\b(?:CODE|ID|REFERENCE|REF|NUMBER|NUM|LABEL):\s*", "", pred_upper)
