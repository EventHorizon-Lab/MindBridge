# ruff: noqa
# mypy: ignore-errors
# Vendored from JingbiaoMei/ATM-Bench@ef4e5dff1a47 (MIT).
#!/usr/bin/env python3
...
RE_COMPACT_DATE = re.compile(r"\b((?:19|20)\d{2})(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\b")
...
RE_CURRENCY_WORDS = re.compile(r"\b(dollars?|usd|pounds?|gbp|euros?|eur)\b", re.IGNORECASE)
RE_NUMBER = re.compile(r"\b\d{1,3}(?:,\d{3})*(?:\.\d+)?\b")
RE_CODE = re.compile(r"\b[A-Z0-9]{4,}\b", re.IGNORECASE)
RE_TIME_AMPM_DOTS = re.compile(r"\b(\d{1,2})(?::(\d{2}))?\s*(a\.m\.|p\.m\.)\b", re.IGNORECASE)
...
def remove_date_time_text(text: str) -> str:
    ...
    cleaned = cleaned.replace("noon", " ").replace("midday", " ").replace("midnight", " ")
...
def location_token_match(ground_truth: str, prediction: str) -> bool:
    ...
    if not any(token in ADDRESS_KEYWORDS for token in gt_tokens):
        return False
    filtered = [token for token in gt_tokens if token not in STOPWORDS and not token.isdigit()]
    pred_tokens = set(tokenize(prediction))
    if not filtered:
        return False
...
def semantic_units_match(ground_truth: str, prediction: str) -> bool:
    ...
    if gt_dates:
        ...
        if not any(gt_d[-4:] == pr_d[-4:] for gt_d in gt_date_vals for pr_d in pr_date_vals):
            return False
