...
async def run_egotempo_clip(
    ...
    *,
    ...
    try:
        await memory.add(
            video_path,
            metadata={"benchmark": "egotempo", "clip_id": questions[0].clip_id},
        )
        ingest_failures = 0
    except Exception:
        ingest_failures = 1
    ...
    for question, outcome in zip(questions, answered, strict=True):
        if not isinstance(outcome, BaseException):
            ...
            results.append(_failed_result(question, _error_code(outcome), ingest_failures))
        else:
            raise outcome
...
async def _answer_question(
    ...
    try:
        async with semaphore:
            result = await memory.ask(prompt, limit=recall_limit)
