...
def test_a_merge_over_a_dead_row_keeps_every_vector_bound_to_its_own_id(
    ...
    then merged agreed with an exhaustive cosine over its own stored vectors on 0.15 of its top
    100 instead of 0.99.
    ...
    with ZvecIndex(path, dimension=2) as index:
        ...
        index.flush()
        # The merge every maintenance path routes through.
        index.optimize()
        index.flush()
...
def test_only_a_collection_left_clean_is_merged_in_place_by_its_next_owner(
    ...
    marker -- because the last one deleted something, or was killed -- copies. Without that, every
    session that merged rewrote the whole collection, forever: 2.13 s against 0.54 s on a 48 MB
    store, and about 21 s on a 481 MB one.
    """
    _require_zvec()
    path = tmp_path / "index"
