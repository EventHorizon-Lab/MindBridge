...
def test_consolidation_forgetting_refuses_a_bound_naming_assertion(tmp_path: Path) -> None:
    ...
    with _identity_memory(tmp_path / "consolidated", consolidator) as memory:
        ...
        assert memory.identity(identity_id).name == "Li"  # type: ignore[union-attr]
        assert _asserted_name(memory, identity_id) == "Li"
        # The kernel's own cross-modal merge logs a row of its own here; what must be absent
        # is a CONSOLIDATE row, because the proposal was refused before it applied.
        assert [item.operation.intent for item in memory.operations()] == [
            MemoryIntent.IDENTIFY,
            MemoryIntent.MERGE,
        ]
...
def test_every_operation_payload_field_is_derived_from_the_dataclass() -> None:
    ...
    from mindbridge.control import _spatial_payload, proposal_payload
    ...
    assert {field.name for field in dataclasses.fields(proposal)} == set(proposal_payload(proposal))
...
def test_registering_a_name_again_after_rolling_it_back_names_the_person(tmp_path: Path) -> None:
    ...
    with _identity_memory(tmp_path / "re-register", consolidator) as memory:
        ...
        assert _asserted_name(memory, identity_id) == "Li"
        assert memory.identity(identity_id).name == "Li"  # type: ignore[union-attr]
        # Both attempts are auditable, and exactly one of them still stands. The kernel's own
        # cross-modal merge logs a row of its own beside them.
        rows = [row for row in memory.operations() if row.operation.intent is MemoryIntent.IDENTIFY]
        assert len(rows) == 2
