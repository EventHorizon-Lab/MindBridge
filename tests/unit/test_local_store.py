...
import mindbridge.infrastructure.local.store._codec as codec_module
import mindbridge.infrastructure.local.store._identity as identity_module
...
def test_exemplar_bank_ranks_identities_like_the_per_row_scan_did() -> None:
    ...
    with closing(sqlite3.connect(":memory:")) as connection:
        ...
        connection.executemany(
            ...
            [
                ("id-a", 0, codec_module.pack_vector((1.0, 0.0)), "t1"),
                ("id-b", 0, codec_module.pack_vector((0.6, 0.8)), "t2"),
                ("id-b", 1, codec_module.pack_vector((0.0, 1.0)), "t3"),
                ("id-c", 0, codec_module.pack_vector((2.0, 0.0)), "t4"),
            ],
        )
        rows = connection.execute(
    ...
    bank = identity_module._ExemplarBank(rows, dimension=2, modality="voice")
...
def test_write_speech_enrols_only_the_speaker_centroids_a_turn_uses(tmp_path: Path) -> None:
    ...
    with LocalStore(tmp_path) as store:
        store.records.write_memories((memory,))
        segments = store.media.write_speech(
            asset.asset_id,
            SpeechAnalysis(
